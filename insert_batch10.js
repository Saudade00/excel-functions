...
// 生成要插入的文本（JS 格式，与现有格式一致）
function objToJS(obj) {
  const entries = Object.entries(obj);
  let s = '';
  entries.forEach(([k, v], i) => {
    s += '  "' + k + '": ' + JSON.stringify(v, null, 2).replace(/\n/g, '\n  ') + (i < entries.length - 1 ? ',' : '') + '\n';
  });
  return s;
}
...
// lastBracket 是 ] 的位置，后面是换行
// 插入位置 = qStart + qMarker.length + lastBracket + 1（即 ] 后面的位置）
...
// 跳过 JS 语法验证（QUIZ_DATA 对象太大，new Function 解析不稳定）
// 直接写回文件，然后用浏览器打开验证
console.log('⚠️ 跳过 JS 语法验证，直接写入文件');
console.log('请随后用浏览器打开文件确认无 JS 错误');
