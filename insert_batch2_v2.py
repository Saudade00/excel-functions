...
new_quiz_data = r"""
  ...
  "NETWORKDAYS": [
    { type:"choice", question:"=NETWORKDAYS(\"2024-03-01\", \"2024-03-31\") 的结果大约是？（假设无节假日）", options:["21","31","22","20"], answer:0, explanation:"NETWORKDAYS 计算两个日期之间的工作日数（不含周末）。3月有 31 天，去掉 10 天周末 ≈ 21 个工作日。" },
...
def find_insert_position(html):
    ...
    if pos == -1:
        return -1, "未找到 QUIZ_DATA"
    
    # 找到第一个 {
    brace_start = html.find('{', pos)
    if brace_start == -1:
        return -1, "未找到 QUIZ_DATA 对象的开头"
    ...
    while i < len(html):
        ...
        return -1, "未找到 QUIZ_DATA 对象的结束位置"
    ...
    while j < len(obj_content):
        ...
        return -1, "未找到 SUMPRODUCT 数组的结束"
    
    # 插入位置是 SUMPRODUCT 数组结束后、下一个键值对之前
    # 或者直接返回 sumproduct_end + 1（SUMPRODUCT 的 ] 之后）
    # 但需要确保后面是 ,\n 或 }\n
    insert_pos = sumproduct_end + 1
    
    # 检查后面是否需要添加逗号
    after = html[insert_pos:insert_pos+10]
    if after.strip().startswith('}'):
        # 后面是 }，说明 SUMPRODUCT 是最后一个条目，需要加逗号
        return insert_pos, "need_comma"
    else:
        return insert_pos, "no_comma"
...
print("查找插入位置...")
insert_pos, comma_flag = find_insert_position(html)
if insert_pos == -1:
    print(f"错误：{comma_flag}")
    exit(1)

print(f"找到插入位置：字符 {insert_pos}")
print(f"状态：{comma_flag}")

# 准备要插入的数据
if comma_flag == "need_comma":
    data_to_insert = ',\n' + new_quiz_data.strip() + '\n'
else:
    data_to_insert = '\n' + new_quiz_data.strip() + ',\n'
