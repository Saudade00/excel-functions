...
details = {
  ...
  "SUMIF": {
    ...
    "params": "range：条件判断的区域。criteria：条件（如 \">60\"、\"男\"）。sum_range：可选，实际求和的区域（省略时对国家 range 求和）。",
...
# 构建替换映射：将 "usage":"", "params":"", "notes":"", "example":"", "rating":"", "stars":"", "hasDetail":false
# 替换为完整内容
updated = 0
skipped = []
...
for fname, d in details.items():
    # 构建新的字段字符串（注意用 \\n 让输出文件里是 \n 两个字符）
    new_block = f'"usage":"{d["usage"]}","params":"{d["params"]}","notes":"{d["notes"]}","example":"{d["example"]}","rating":"{d["rating"]}","stars":"{d["stars"]}","hasDetail":true'
    # 匹配原来的空字段（或已有部分内容的）
    # 模式：查找 "name":"fname", ... 后面跟着要替换的字段
    pattern = re.escape(f'"name":"{fname}"')
    
    # 在 content 中找这个函数条目
    # 找法是：从 "name":"fname" 开始，到下一个 "name":" 或 数组结尾
    # 更简单的做法：直接替换 "usage":"","params":"","notes":"","example":"","rating":"","stars":"","hasDetail":false
    old = f'"usage":"","params":"","notes":"","example":"","rating":"","stars":"","hasDetail":false'
    new = f'"usage":"{d["usage"]}","params":"{d["params"]}","notes":"{d["notes"]}","example":"{d["example"]}","rating":"{d["rating"]}","stars":"{d["stars"]}","hasDetail":true'
    ...
    if old in content:
        ...
        # 也许 usage 已有内容但不是我们想要的格式，尝试更通用的替换
        # 用正则替换整段
        # 找 "name":"fname", 然后替换后面的字段
        regex = re.compile(
            r'("name":"' + re.escape(fname) + r'".*?"usage":).*?("params":).*?("notes":).*?("example":).*?("rating":).*?("stars":).*?("hasDetail":).*?([,}])',
            re.DOTALL
        )
        m = regex.search(content)
        if m:
            # 用更精确的方式：直接对该函数条目做全文替换
            print(f'  ⚠ {fname}: 已有内容，跳过（需手动检查）')
            skipped.append(fname)
        else:
            print(f'  ✗ {fname}: 未找到匹配条目')

print(f'\n共更新 {updated} 个函数，跳过 {len(skipped)} 个')
if skipped:
    print(f'跳过：{skipped}')
...
# 验证：检查 JS 语法（查找实际换行符）
import re
# 检查 FUNCTIONS 数组中是否有实际换行符（ASCII 10）在引号内
# 简单检查：在 "notes":" 和 " 之间是否有实际换行
bad_newlines = re.findall(r'"notes":"[^"]*[\r\n][^"]*"', content[content.find('const FUNCTIONS'):content.find('const FUNCTIONS')+5000])
if bad_newlines:
    print(f'⚠ 警告：发现 {len(bad_newlines)} 处可能有非法换行符')
else:
    print('✓ 未发现有非法换行符')
